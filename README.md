 ...
 **snuggsiツ** believes in using [Progressive Enhancement](https://en.wikipedia.org/wiki/Progressive_enhancement). Not just with your code, but also with your Developer eXperience _(DX)_. We will start off with a simple Custom Element and gradually enhance functionality jtep-by-step.
...
```
 👍 Rule of thumb: _If the `{token}` name is [not in a thesaurus](https://en.wikipedia.org/wiki/Metasyntactic_variable) I probably shouldn't use it. Not for coputers sake but for humans._
